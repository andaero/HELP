    ...
    for name in dataset_names:
        for model in model_names:
            for simThreshold in simThresholds:
                ...
                pattern_df.to_csv(f'results_new/{name}/weighted_merge_{name}_{folder_name}_center_rebalance_{model}_sim_{simThreshold}.csv', index=False)
        print(pattern_df)
...
def generateVectors(raw_logs, model_name):
    ...
    for raw_log in raw_logs:
        ...
        words = log.split()
        num_words = len(words)
        # list with the length of each word
        word_lens_str = ", ".join(str(len(word)) for word in words)
        print(word_lens_str)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_wc', action='store_true', default=False, help='Do not use word count')
    ...
    simThresholds = [0.9]
    load_data(datasets, models, simThresholds, folder_name)
