...
def evaluate(dataset, output_path, groundtruth, parsedresult):
...
def evaluate_models(dataset, output_path, path, models, max_logs=10):
    groundtruth = f'../../full_dataset/{dataset}/{dataset}_full.log_structured.csv'

    df_groundtruth = pd.read_csv(groundtruth)


    final_out = os.path.join(output_path, f'{dataset}Results.csv')
    ...
    for model in models:
        parsedresult = f'results_parsed_hyp_scan_wc/{dataset}/{path}_parsed_{model}.csv'
    ...
    if not existing_df.empty:
        # check if 'Model' column exists in the existing dataframe
        if 'Model' not in existing_df.columns:
            # set model for existing columns to gpt-4o
            existing_df['Model'] = 'gpt-4o'
        # Remove existing entries for the models being evaluated if same max_logs and model
        existing_df = existing_df[~((existing_df['Model'].isin(models)) & (existing_df['Logs'] == max_logs))]
        # Concatenate the existing dataframe with the new results
        res_df = pd.concat([existing_df, res_df], ignore_index=True)

    # Save the results
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    res_df.to_csv(final_out, index=False)
